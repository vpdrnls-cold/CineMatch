...
CineMatch는 사용자가 몇 편의 영화를 평가하면, **MovieLens의 기존 사용자 중 취향이 가장 비슷한 사용자를 찾아 그들의 평점을 기반으로 영화를 추천하는 개인화 영화 추천 시스템**

단순히 영화의 평균 평점을 보여주는 것이 아니라,
**새로운 사용자의 평점 → 유사 사용자 탐색 → 유사 사용자의 선호 영화 분석 → 예상 평점 계산 → 추천**의 과정을 거쳐 개인화된 추천 결과 제공
...
사용자가 처음 제시된 영화 중 본 영화에 평점을 입력하면 새로운 사용자 프로필 생성

평점을 입력하지 않은 영화는 분석에서 제외하고, 실제로 평가한 영화만 사용자의 취향을 나타내는 데이터로 사용

### 2. Find Similar Users

새로운 사용자가 평가한 영화와 기존 사용자가 평가한 영화 중 **공통 영화가 최소 3개 이상인 사용자**만 대상으로 유사도 계산

그리고 공통으로 평가한 영화들의 평점 벡터에 **Cosine Similarity**를 적용해 사용자 간 유사도를 구함.
...
선택된 유사 사용자들이 **3.5점 이상** 평가한 영화만 추천 후보로 사용.

또한 유사 사용자 중 **최소 3명이 평가한 영화**만 후보로 남겨 특정 사용자 한 명의 평가에 지나치게 의존하지 않도록함.

이미 새로운 사용자가 평가한 영화는 추천 후보에서 제외

### 4. Predicted Rating

각 영화에 대해 유사 사용자의 평점을 유사도로 가중하여 예상 평점 계산
...
즉, **나와 더 비슷한 사용자의 평점에 더 큰 영향력**을 주는 방식

예상 평점이 높은 순으로 정렬한 뒤 최종적으로 Top 10 영화 추천
...
평점은 **0.5점 단위의 0.5 ~ 5.0점**으로 구성되어 있고,

데이터는 MovieLens에서 제공하는 공개 데이터셋을 사용함.
...
이를 통해 모델이 단순히 영화를 추천하는 것에서 끝나는 게 아니라, **사용자가 실제로 선호한 영화와 얼마나 겹치는지** 확인할 수 있도록 구성함.
...
프로젝트 루트에 `.env` 파일을 생성하고 TMDB API Token 추가
...
브라우저에서 CineMatch를 실행하면 영화 평점을 입력하고 개인화된 추천 결과 확인 가능
